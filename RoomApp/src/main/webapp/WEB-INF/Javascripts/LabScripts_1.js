...
//	alert(id+"  sub  "+subcategoryId+"  categoryId     "+categoryId+"     aount     "+amount);
	try {
...
//				if(totalAmount > 0)
//				else document.getElementById('lbl_totalAmount').innerHTML = '0';
			}
		}else{
			if(totalAmount != undefined){
...
function addDiv(subCategoryName){
	try {
...
function labEntryResultSubmit(byPass){
	...
	try {
		...
		for(var index = 0;index < resultFig.length;index++){
			...
			outString += ''+document.getElementById('hdCategory_'+IdValue).value+'`'+(resultFig[index].value).trim()+'`'+document.getElementById('hdUnits_'+IdValue).value+'`'+document.getElementById('hdMaleRange_'+IdValue).value+'`'+document.getElementById('hdFeMaleRange_'+IdValue).value+'@';
...
function labEntrySaveWithAll(byPass){
	...
	try {
		...
		for(var index = 0;index < resultFig.length;index++){
			...
			if(byPass == 1){
				if(flag == 0)outString += ''+document.getElementById('hdCategory_'+IdValue).value+'`'+(resultFig[index].value).trim()+'`'+document.getElementById('hdUnits_'+IdValue).value+'`'+document.getElementById('hdMaleRange_'+IdValue).value+'`'+document.getElementById('hdFeMaleRange_'+IdValue).value+'@';
			}else if(byPass == 2){
				outString += ''+document.getElementById('hdCategory_'+IdValue).value+'`'+(resultFig[index].value).trim()+'`'+document.getElementById('hdUnits_'+IdValue).value+'`'+document.getElementById('hdMaleRange_'+IdValue).value+'`'+document.getElementById('hdFeMaleRange_'+IdValue).value+'@';
			}
		}
